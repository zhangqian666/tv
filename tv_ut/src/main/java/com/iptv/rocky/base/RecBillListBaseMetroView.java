...
import com.iptv.rocky.view.RecBillMetroItemView;

public abstract class RecBillListBaseMetroView extends RelativeLayout {
	...
	public RecBillListBaseMetroView(Context context) {
		this(context, null, 0);
	}
	
	public RecBillListBaseMetroView(Context context, AttributeSet attrs) {
		this(context, attrs, 0);
	}
	
	public RecBillListBaseMetroView(Context context, AttributeSet attrs, int defStyle) {
	...
	public void addMetroItem(BaseMetroItemData itemData) {
		RecBillMetroItemView itemView = new RecBillMetroItemView(getContext());
	...
	public void destroy() {
		...
		for (int i = 0; i < count; i++) {
			...
			if (child instanceof RecBillMetroItemView) {
				RecBillMetroItemView itemView = (RecBillMetroItemView) child;
				itemView.destroy();
			}
		}
	}
	...
	protected RelativeLayout.LayoutParams setItemPosition(BaseMetroItemData itemData) {
		...
		int width = (int)(itemData.widthSpan * ScreenUtils.getLiveItemWidth());
