...
import com.iptv.common.data.EnumType.MyHotelSubContentType;
...
public class MyHotelSecLayoutFactory {
...
//			return initData(new LiveTypeBackImageData(), page);
	 ...
	 * 直播
	*/
	public static HomeItemData createLiveData(MyHotelSec info) {
	...
	private static HomeItemData createItemData(MyHotelSec info) {
    ...
    private static HomeItemData initData(HomeItemData data, MyHotelSec info, double widthSpan, double heightSpan) {
        data.id = info.id;
        data.pageItem = new HomePageItem();
        data.pageItem.id = info.id;
        Log.d("MyHotelSecLayoutFactory", "type_id:"+info.type_id);
        data.pageItem.contentid = info.type_id;
        data.pageItem.contenttype = info.content_type;
        data.pageItem.myHotelSubContentType= info.my_hotel_sub_content_type;
