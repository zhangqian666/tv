...
package com.iptv.rocky.view.gallery;
...
public class Gallery extends AbsSpinner implements GestureDetector.OnGestureListener {
    ...
    public Gallery(Context context, AttributeSet attrs, int defStyle) {
        ...
        TypedArray a = context.obtainStyledAttributes(
                attrs, R.styleable.TvGallery, defStyle, 0);

        int index = a.getInt(R.styleable.TvGallery_gravity, -1);
        if (index >= 0) {
            setGravity(index);
        }

        int animationDuration =
                a.getInt(R.styleable.TvGallery_animationDuration, -1);
        if (animationDuration > 0) {
            setAnimationDuration(animationDuration);
        }

        int spacing =
                a.getDimensionPixelOffset(R.styleable.TvGallery_spacing, 0);
        setSpacing(spacing);

        float unselectedAlpha = a.getFloat(
                R.styleable.TvGallery_unselectedAlpha, 0.5f);
...
//        mGroupFlags |= FLAG_SUPPORT_STATIC_TRANSFORMATIONS;
        ...
        /**
         * because can't access mGroupFlags, so call this method
         * @author PennerYu
         */
    ...
    protected boolean isAutoMargin() {
    	return true;
    }
    ...
    private void scrollIntoSlots() {
        ...
        if (isAutoMargin() || index != 0) {
         	scrollAmount = getCenterOfGallery() - getCenterOfView(mSelectedChild);
        } else {
         	scrollAmount = getLeftOfGallery() - getLeftOfView(mSelectedChild);
        }
...
//        int numItems = mItemCount;
    ...
    private boolean scrollToChild(int childPosition) {
        ...
        if (child != null) {
        	int distance = 0;
        	if (isAutoMargin() || childPosition != 0) {
        		distance = getCenterOfGallery() - getCenterOfView(child);
        	} else {
        		distance = getLeftOfGallery() - getLeftOfView(child);
        	}
