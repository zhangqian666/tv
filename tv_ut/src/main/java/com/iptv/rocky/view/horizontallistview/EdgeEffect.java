...
public class EdgeEffect {
    ...
    public EdgeEffect(Context context, int direction ) {
        ...
        int resId = res.getIdentifier( "tv_hlv_overscroll_edge", "drawable", context.getPackageName() );
        if( resId > 0 ) {
        	mEdge = res.getDrawable(resId);
        } else {
        	throw new IllegalStateException( "Cannot find resource 'tv_hlv_overscroll_edge'" );
        }
        
        resId = res.getIdentifier( "tv_hlv_overscroll_glow", "drawable", context.getPackageName() );
        if( resId > 0 ) {
        	mGlow = res.getDrawable(resId);
        } else {
        	throw new IllegalStateException( "Cannot find resource 'tv_hlv_overscroll_glow'" );
        }
